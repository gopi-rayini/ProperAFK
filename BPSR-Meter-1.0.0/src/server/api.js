...
function initializeApi(app, server, io, userDataManager, logger, globalSettings) {
    ...
    app.get('/api/position', (req, res) => {
        try {
            const pos = userDataManager.getLocalPosition();
            if (!pos) return res.status(404).json({ code: 1, msg: 'no position' });
            res.json({ code: 0, pos });
        } catch (e) {
            res.status(500).json({ code: 2, msg: 'error', err: String(e) });
        }
    });
 // Ajustar la ruta
    ...
    // push live position at 20Hz
    setInterval(() => {
        const pos = userDataManager.getLocalPosition();
        if (pos) io.emit('position_update', pos);
    }, 50);
setInterval(() => {
